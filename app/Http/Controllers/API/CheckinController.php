...
class CheckinController extends Controller
{
    ...
    public function selfCheckin(Request $request, SiteSetting $gym)
    {
        try {
            ...
            $validation = $this->checkinService->validateCheckin($this->user, $gym);

            if (!$validation['valid']) {
                return failureResponse($validation['message'], 400);
            }

            if ($this->checkinService->hasRecentCheckin($this->user->id, $gym->id)) {
            ...
            $message = $validation['warning'] ?? false 
                ...
                : 'Check-in successful! Welcome to ' . $gym->gym_name;

            return successResponse($checkin, $message);
     ...
     */
    public function getUserCheckinHistory(Request $request, SiteSetting $gym)
    {
        try {
            if (!Auth::guard('sanctum')->check()) {
                return failureResponse('Authentication required.', 401);
            ...
            $history = $this->checkinService->getUserCheckinHistory($user->id, $gym->id, $limit);

            return successResponse($history, 'Check-in history retrieved successfully.');
